...
import { IconActivity, IconZap, IconStar, IconAlertTriangle, IconTrendingUp, IconMusic, IconFilter } from '../components/Icons';
...
const TYPE_META = {
  ...
  interview: { icon:'🎙', color:'#1abc9c',   label:'Press',      bg:'rgba(26,188,156,0.08)'  },
  ...
  milestone2:{ icon:'⭐', color:'#f1c40f',   label:'Milestone',  bg:'rgba(241,196,15,0.08)'  },
};
...
export default function SocialFeed({ game }) {
  ...
  const trendItem = feed.find(f=>f.type==='trend');
  ...
  return (
    <div className="page" style={{ paddingBottom: 80 }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
          <div>
            ...
            <p style={{ color:'var(--text-muted)', fontSize:13, marginTop:3 }}>
              ...
              {importantCount > 0 && <span style={{ color:'#f39c12', marginLeft:8 }}>🔔 {importantCount} important</span>}
            </p>
          </div>
        </div>
        ...
        {/* Trend banner */}
        {currentTrend && (
          <div style={{ marginTop:10, padding:'8px 12px', background:'rgba(201,168,76,0.06)', border:'1px solid rgba(201,168,76,0.2)', borderRadius:'var(--radius)', display:'flex', alignItems:'center', gap:8 }}>
            <span style={{ fontSize:14 }}>📈</span>
            <div>
              <span style={{ fontSize:11, color:'var(--gold-dim)', fontFamily:'var(--font-display)', letterSpacing:'0.1em', textTransform:'uppercase' }}>TRENDING NOW</span>
        ...
        {filtered.map((item, i) => {
          ...
          return (
            <div key={item.id||i} style={{
              ...
              margin: item.important ? '0 -16px', padding: item.important ? '12px 16px' : '12px 0',
            }}>
              {/* Icon */}
              <div style={{
...
// Import the genre trend cycles for display
