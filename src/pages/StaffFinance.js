...
import { IconCheck, IconDollarSign, IconTrash } from '../components/Icons';
      ...
      {tab==='staff' && (
        <div>
          {STAFF_ROLES.map(role=>{
            ...
            const maxed = cur>=role.levels.length;
            ...
            return (
              <div key={role.id} style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'var(--radius-lg)', padding:16, marginBottom:12 }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}>
                  <div>
                    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
                      ...
                      {cur>0&&<span style={{ fontSize:9, padding:'1px 6px', background:'var(--gold-glow)', border:'1px solid var(--border-2)', color:'var(--gold)', borderRadius:2, fontFamily:'var(--font-display)', letterSpacing:'0.08em' }}>LVL {cur}</span>}
                    </div>
                ...
                <div style={{ display:'flex', gap:4, marginBottom:8 }}>
                  {role.levels.map((_,i)=>(
                    <div key={i} style={{ flex:1, height:4, borderRadius:2, background:i<cur?'var(--gold)':'var(--bg-3)' }} />
                  ))}
                </div>
                {cur>0&&<div style={{ fontSize:12, color:'#2ecc71', marginBottom:8 }}>✓ {role.levels[cur-1].bonus}</div>}
                {next ? (
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <div>
                      ...
                      <div style={{ fontSize:11, color:'var(--text-muted)' }}>{fmtM(next.weeklyCost)}/wk ongoing</div>
                    </div>
                    <button className={`btn btn-sm ${game.cash>=cost?'btn-gold':'btn-outline'}`} onClick={()=>hireStaff(role.id)} disabled={game.cash<cost} style={{ opacity:game.cash>=cost?1:0.5 }}>
                      Hire — {fmtM(cost)}
                    </button>
                  </div>
                ) : (
            ...
            <div style={{ marginBottom:16 }}>
              ...
              {(game.activeLoans||[]).filter(l=>!l.paid&&l.type!=='equity').map(loan=>(
                <div key={loan.id} style={{ background:'rgba(231,76,60,0.06)', border:'1px solid rgba(231,76,60,0.2)', borderRadius:'var(--radius)', padding:'10px 14px', marginBottom:8 }}>
                  ...
                  {loan.missedPayments>0&&<div style={{ fontSize:11, color:'#e74c3c', marginTop:3 }}>⚠️ {loan.missedPayments} missed payment{loan.missedPayments>1?'s':''}</div>}
              ...
              {(game.training||[]).map(t=>(
                <div key={t.id} style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'var(--radius)', padding:'10px 14px', marginBottom:8 }}>
                  <div style={{ display:'flex', justifyContent:'space-between' }}>
                    ...
                    <div style={{ textAlign:'right' }}>
                      <div style={{ fontFamily:'var(--font-mono)', fontSize:13, color:'var(--gold)' }}>{t.weeksLeft}wk</div>
