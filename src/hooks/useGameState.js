...
function checkObjectives(state, done) {
  ...
  const vals = {
    ...
    totalRevenue: state.totalRevenue,   // earn_500k, earn_1m, earn_5m etc all use this
  ...
  const advanceWeek = useCallback(() => {
    ...
    setGame(prev => {
      ...
      releases = releases.map(r => {
        ...
        // Streams: decay naturally — difficulty affects revenue rate only
        const ws = Math.floor(r.weeklyStreams * decay * tBonus * mktMult * noise);
        // Revenue: difficulty controls $ per stream
      ...
      // Insert your releases — low threshold so early artists chart
      ...
      // ── 19. MARKET REFRESH ───────────────
      // Refresh on week 1 (init), and every 4 weeks after.
      // Use prev.week so week 4,8,12... all trigger correctly.
      // Also always refresh if market is empty or stale (< 4 artists)
      ...
      if (shouldRefresh) {
        ...
        newFeed.push({ id:uuidv4(), text:`🔄 Talent market refreshed — new artists available to sign.`, week:prev.week+1, type:'news', important:false });
      }
